// Decryption method for the McEliece cryptosystem.
// takes as input any file, a private key, and and the name of an output file (in that order)
// the private key must be in the format generated by keygen.c, else throws an error
...
void errorlocator(const int m, const int t, unsigned char codeword[1 << (m-3)], int goppa[t+1], int private_perm[1 << m], int sigma[t+1]) {
    ...
    int A2[t+1]; // this is the even part of the error locator poly
    ...
    // xB2 is just x*B2, i.e. the odd part of the error locator poly
    int xB2[t+1];
...
// start from the 7-th i.e. last bit of a char
    for (int i = 0; i < (1 << m) ; i++) {
...
int main(int argc, char *argv[]) {
    ...
    unsigned char* cleartext = malloc(sizeof(char)*(((file_size-2)/(1 << (m-3)))*((1<<m)-m*t)/8)+1);
    unsigned char* ptr_text = &cleartext[0];
    unsigned char* ptr_buf = &buffer[0];
    int shift = 0;
    memset(cleartext, 0, sizeof(char)*(((file_size-2)/(1 << (m-3)))*((1<<m)-m*t)/8)+1);
    // now decrypt the ciphertext 2^{m-3} chars at a time, which is guaranteed to divide the total amount.
    // if not, then the encrypted file is in the wrong format
    if ((file_size-2) % (1 << (m-3)) != 0) {
